...
def obvod(frame):
    ...
    for i in range(len(colors_name)):
        ...
        image_pu.publish(bridge.cv2_to_imgmsg(obrab_hsv, 'mono8'))
...
for i in range(len(polet_points)):
    save_telem()
    poi = polet_points[i]
    navigate_wait(x=poi[0], y=poi[1], z=2, speed=1.0, frame_id='aruco_map')
    save_telem()
    rospy.sleep(0.2)
    metks = map_cm()
    print(metks)
    for met in metks:
        if met[0][0] < 0 or met[0][1] < 0:
            continue
        if met[0][0] > 4 or met[0][1] > 3.2:
            continue
        if (met[0][0] > 0 and met[0][0] < 4) or (met[0][1] > 0 and met[0][1] < 3.2):
            print(met)
            metks = []
            metks.append(met)
            bre = 1
...
for i in range(10):
    save_telem()
    navigate_wait(x=last_metk[0][0][0], y=last_metk[0][0][1], z=2, speed=1.0, frame_id='aruco_map')
    save_telem()
    rospy.sleep(2)
    metks = map_cm()
    print(metks)
